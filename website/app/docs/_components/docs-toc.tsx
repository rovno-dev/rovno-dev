...
/**
 * Scroll-spy driven TOC.
 * - Desktop (lg+): sticky sidebar, active item highlighted.
 * - Mobile (<lg): floating action button bottom-right, expands into a
 *   scrollable panel of the same headings.
 *
 * Uses IntersectionObserver with a top-of-viewport band so the "active"
 * heading is the last one to cross the band, which matches how readers
 * perceive the current section.
 */
export function DocsTOC({ headings }: DocsTOCProps) {
  ...
  useEffect(() => {
    ...
    // Track the last heading whose top has crossed the "top band" line.
    // Handles the case where two headings are briefly visible.
    const onScroll = () => {
      const band = 120; // px from the top of the viewport
  ...
  // Close mobile menu when a link is tapped or Escape is pressed.
  useEffect(() => {
      ...
      <div className="lg:hidden">
        ...
        >
          ...
          <span className="truncate">
            {activeHeading ? activeHeading.text : "Содержание"}
          </span>
        </button>
