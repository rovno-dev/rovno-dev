...
import { ChevronDown, ChevronUp, Sparkles, Save, Rocket, Trash2 } from "lucide-react";
...
export function ArticleEditorForm({ initial }: { initial?: Article | null }) {
  ...
  // LLM context: the AI panel accepts an arbitrary JSON blob. Known keys are
  // merged into the form; the entire string is persisted to raw_json on save
  // regardless. This is intentionally forgiving — AI tools emit inconsistent
  // shapes and rejecting them client-side just adds friction.
  const applyAiJson = () => {
  ...
  const handleSave = async (status: "draft" | "published") => {
    ...
    try {
      ...
      // Newly-created articles navigate to their edit page so subsequent saves
      // are PATCHes, not duplicate POSTs.
      if (!isEdit) {
  ...
  const handlePublishToggle = async () => {
    ...
    try {
      const updated =
        initial.publication_status === "published"
          ? await unpublishArticle(initial.slug)
          : await publishArticle(initial.slug);
      toast.success(
        updated.publication_status === "published"
          ? t("editor.published")
          : t("editor.unpublished")
      );
      router.refresh();
    } catch (err: any) {
      toast.error(err.message || t("editor.save_failed"));
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {isEdit && (
            <>
              <Button variant="outlined" onClick={handlePublishToggle} disabled={saving}>
