...
import {
  Plus, PencilSimple, Trash, ArrowSquareOutIcon, Buildings, ArrowClockwiseIcon, CircleNotchIcon,
...
export default function AdminClientsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({ kind: "error", message: err?.message || "Не удалось загрузить компаний" });
    }
  }, []);
  ...
  const handleDelete = async (c: ClientListItem) => {
    if (!confirm(`Удалить компанию «${c.name}»? Проекты останутся, но потеряют привязку.`)) return;
    try {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-body-3 text-(--on-bg-medium)">
              {state.kind === "ready" ? `${clients.length} компаний` : "Загрузка…"}
            </p>
          </div>
