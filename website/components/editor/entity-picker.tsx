...
export function EntityPicker({
  ...
  placeholder = "Выбрать…", emptyText = "Ничего не найдено",
  createNoun, onCreate, disabled = false, className,
}: Props) {
  ...
  const canCreate = !!onCreate && draft.trim().length > 0 && !exactMatch && !creating;
    ...
    <div className={cn("relative", className)}>
      ...
      >
        {filtered.length === 0 && !canCreate && (
          <p className="px-3 py-3 text-center text-body-5 text-(--on-bg-low)">{emptyText}</p>
        )}
        {filtered.map((item, idx) => (
          ...
          >
            ...
            {item.meta && (
              <span className="text-[11px] text-(--on-bg-low) tabular-nums shrink-0">{item.meta}</span>
            )}
          </button>
        ))}
        {canCreate && (
