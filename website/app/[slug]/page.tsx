...
import {
  fetchTeamMemberByUsername,
  fetchTeamMemberProjectsPublic,
...
/** Merge DB-pinned projects with the hardcoded fallback list. DB wins. */
...
export default function ExpertPage({
  ...
  const [dbProjects, setDbProjects] = useState<TeamMemberPublicProject[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([
      fetchTeamMemberByUsername(slug).catch(() => null),
      fetchTeamMemberProjectsPublic(slug).catch(() => []),
    ]).then(([info, projects]) => {
      if (cancelled) return;
      setTeamInfo(info);
  ...
  const heroImage = teamInfo?.cover_url || expert.cover || expert.avatar;
  const projects = mergeProjects(dbProjects, expert.projects);
  const projectWord =
    projects.length === 1
      ? "кейс"
      : projects.length < 5
  ...
  return (
    <main className="min-h-screen bg-(--bg) pb-24 md:pb-32">
      ...
      <Container variant="full-width" className="pt-4 md:pt-8">
        <div className={CONTENT_MAX}>
          <div className="relative w-full h-[560px] md:h-[680px] rounded-5xl md:rounded-7xl overflow-hidden bg-(--card) border border-(--outline) animate-reveal">
            ...
            {/* Name + role + location, anchored at the bottom */}
      ...
      {/* ═══════════ STATS ═══════════ */}
      {expert.stats && expert.stats.length > 0 && (
          ...
          <div className={CONTENT_MAX}>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-(--outline) border border-(--outline) rounded-3xl overflow-hidden">
              {expert.stats.map((s, i) => (
                <div
                  key={i}
                  className="bg-(--bg) p-6 md:p-8 animate-reveal fill-mode-both"
        ...
        >
          <div className="space-y-5 animate-reveal">
            ...
            {expert.longBio?.map((para, i) => (
              <p
                key={i}
                className="text-body-2 text-(--on-bg-medium) leading-relaxed"
              >
                {para}
              </p>
            ))}
      ...
      {/* ═══════════ MILESTONES / TIMELINE ═══════════ */}
      {expert.milestones && expert.milestones.length > 0 && (
        <ScrollReveal threshold={0.05}>
          <Container variant="full-width" className="pt-20 md:pt-28">
            <div className={CONTENT_MAX}>
              <h2 className="text-display-4 md:text-display-3 text-(--on-bg-high) mb-8 tracking-tight">
            ...
            {!loaded && projects.length === 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="relative aspect-[16/10] rounded-3xl border border-(--outline) bg-(--bg-disabled) overflow-hidden"
                  >
            ...
            <div className="relative rounded-5xl border border-(--outline) bg-(--card) p-8 md:p-14 overflow-hidden text-center">
              ...
              <div className="relative">
                ...
                <h2 className="text-display-3 md:text-display-2 text-(--on-bg-high) tracking-tight mb-6 max-w-2xl mx-auto">
                  Хотите работать с {expert.name.split(" ")[0]}?
                </h2>
