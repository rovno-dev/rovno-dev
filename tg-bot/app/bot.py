...
async def dump_last(message: types.Message, command: CommandObject = None):
    limit = int(command.args) if command and command.args and command.args.isdigit() else 10
    with SessionLocal() as session:
...
async def accept_callback(callback: types.CallbackQuery):
    # Safe caption check to avoid NoneType errors
    current_caption = callback.message.caption or ""
    await callback.message.edit_caption(caption=current_caption + f"\n\n✅ Взял: {callback.from_user.first_name}")
    await callback.answer()

async def main():
    # ponytail: Allow Docker network/DNS to settle before initiating polling
    await asyncio.sleep(5)
    await bot.set_my_commands([
        BotCommand(command="start", description="Главное меню"),
        BotCommand(command="dump_last", description="Выгрузить последние N"),
